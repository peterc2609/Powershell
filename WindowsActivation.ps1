...
function Get-ProductKey {
    ...
    $isOEMKey = $digitalProductId.Length -eq 167
    if ($isOEMKey) {
        # Offset start if DigitalProductId is found to be the correct length
        $offset = 52
    } else {
        # Use default offset otherwise
        $offset = 52
    }
    ...
    for ($i = 24; $i -ge 0; $i--) {
        $cur = 0
        for ($j = 14; $j -ge 0; $j--) {
            $cur = ($cur * 256) -bxor $digitalProductId[$j + $offset]
            $digitalProductId[$j + $offset] = [math]::Floor([double]($cur / 24))
            $cur = $cur % 24
        }
        $decodedKey = $keyChars[$cur] + $decodedKey
        if (($i % 5) -eq 0 -and $i -ne 0) {
            $decodedKey = "-" + $decodedKey
        }
    }

    # Output the decoded product key
    Write-Output "Decrypted Windows Product Key: $decodedKey"
}
