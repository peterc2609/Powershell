...
param(
    ...
    [string]$StorageAccountName = "stfalconmgtintuneuks001",

    [parameter(Mandatory = $false, HelpMessage = "Name of the Azure Storage Blob container.")]
    [ValidateNotNullOrEmpty()]
    [string]$ContainerName = "ospreywallpaper",

    [parameter(Mandatory = $false, HelpMessage = "SAS Token for Azure Storage Blob container.")]
    [ValidateNotNullOrEmpty()]
    [string]$SASToken = "?sp=rl&st=2024-01-26T09:49:19Z&se=2034-01-26T17:49:19Z&spr=https&sv=2022-11-02&sr=c&sig=usFR3jXvMXVTNikVKdgHkN0o%2FGGf7c4%2BQ2tw%2B18oZAk%3D"
)
Begin {
    ...
    $RestrictedApplicationPackagesPrincipal = "ALL RESTRICTED APP PACKAGES"
...
Process {
    ...
    function Write-LogEntry {
        param (
            ...
            [ValidateNotNullOrEmpty()]
            [string]$FileName = "Set-OspreyDesktopWallpaper.log"
        )
        ...
        $LogText = "<![LOG[$($Value)]LOG]!><time=""$($Time)"" date=""$($Date)"" component=""OspreyDesktopWallpaper"" context=""$($Context)"" type=""$($Severity)"" thread=""$($PID)"" file="""">"
        ...
        try {
            ...
            Write-Warning -Message "Unable to append log entry to Set-OspreyDesktopWallpaper.log file. Error message at line $($_.InvocationInfo.ScriptLineNumber): $($_.Exception.Message)"
        }
    }
    ...
    if ($AzureStorageBlobContent -ne $null) {
        ...
        # Check if Lenovo wallpaper files are present in the Azure Storage blob and replace those in the default location
        $LenovoWallpaperBlobFiles = $AzureStorageBlobContent | Where-Object { $PSItem.Name -like "ThinkPad-ThinkCentre_wallpaper.png" }
        if ($LenovoWallpaperBlobFiles -ne $null) {
            Write-LogEntry -Value "Detected Lenovo theme wallpaper files in container, will replace matching local Lenovo theme wallpaper file" -Severity 1

            # Remove all items in '%windir%\Web\Wallpaper\Lenovo' (Windows 10) directory and replace with wallpaper content from storage account
            $LenovoWallpaperImagePath = Join-Path -Path $env:windir -ChildPath "Web\Wallpaper\Lenovo"
            $LenovoWallpaperImages = Get-ChildItem -Path $LenovoWallpaperImagePath -Filter "*.png"
            foreach ($LenovoWallpaperImage in $LenovoWallpaperImages) {
                # Remove current theme wallpaper image file
                Remove-WallpaperFile -FilePath $LenovoWallpaperImage.FullName
            }

            foreach ($LenovoWallpaperBlobFile in $LenovoWallpaperBlobFiles) {
                # Download new wallpaper content from storage account
                Invoke-WallpaperFileDownload -FileName $LenovoWallpaperBlobFile.Name -Destination $LenovoWallpaperImagePath
            }
        }
